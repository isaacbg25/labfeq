...
Després de fer la pràctica s'han obtingut els següents resultats.
...
\begin{itemize}
    ...
    \item \textbf{Volum promig: } $\boxed{V = 1616.49 \text{ mL}}$.
\end{itemize}
...
Per calibrar la bomba hem fet un seguit de mesures dels volums corresponents als valors de revolucions per minut (rpm) donats per la bomba. Els valors obtinguts es poden veure a \ref{fig1}.
...
L'equació obtinguda amb els nostres punts experimentals 
és $y=7.0893x-16.071$, amb una $R^2$ de 0.9995, valor que ens indica que les nostres mesures tenen una bona correlació lineal.

A partir d'aquí calculem els cabals corresponents a cada valor de revolucions per minut donades per la bomba, usant que
